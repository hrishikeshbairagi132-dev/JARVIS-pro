...
def fetch_backup_google_data(ticker: str, asset_class: str) -> Optional[pd.DataFrame]:
    try:
        ...
        exch = "NSE" if asset_class == "Equity (NSE)" else "CURRENCY"
        sym = ticker.replace(".NS", "").replace(".BO", "")
        if asset_class == "Crypto": sym = f"{sym}-USD" if not sym.endswith("-USD") else sym
...
def fetch_live_options_matrix(ticker: str, current_price: float, asset_class: str):
    if asset_class == "Crypto" or current_price <= 0: return None, 1.0
    try:
...
# ============================================================
# 📡 7. ULTRA FAST DATA ROUTER
# ============================================================
@st.cache_data(ttl=5, show_spinner=False)
def fetch_data(ticker: str, asset_class: str) -> Optional[pd.DataFrame]:
    ticker = ticker.upper().strip()
    sym = ticker if (ticker.endswith(".NS") or ticker.endswith(".BO") or "-" in ticker) else ticker + ".NS"
    if asset_class == "Crypto": sym = ticker.replace(".", "-") + "-USD" if not ticker.endswith("-USD") else ticker
    try:
...
asset_class = st.sidebar.selectbox("एसेट क्लास", ["Equity (NSE)", "Crypto"])
...
def run_analysis():
    df = fetch_data(ticker_input, asset_class)
    if df is None or df.empty:
        st.error(f"❌ {ticker_input} का लाइव डेटा लोड नहीं हो सका।")
        return
    ...
    if app_mode == "सिंगल स्टॉक एनालिसिस":
        ...
        col4.metric("फ्रेंडशिप स्कोर", f"{f_score:.0f}%")
        
        if asset_class == "Equity (NSE)":
            st.markdown("### 🎰 लाइव ऑप्शन चैन मैट्रिक्स")
            opt_df, pcr_val = fetch_live_options_matrix(ticker_input, last['close'], asset_class)
        ...
        current_signal_text = f"{ticker_input} {last['final_signal']}"
        if "BUY" in last['final_signal']:
            ...
            log_trade(ticker_input, last['final_signal'], last['close'], sl, tp)
            if enable_voice and st.session_state.last_spoken_signal != current_signal_text:
                msg = f"{ticker_input} में खरीदारी का सिग्नल मिला है।" if voice_language == "Pure Hindi (हिंदी)" else f"Buy signal detected for {ticker_input}."
            ...
            if enable_voice and st.session_state.last_spoken_signal != current_signal_text:
                msg = f"{ticker_input} अभी होल्ड पर है।" if voice_language == "Pure Hindi (हिंदी)" else f"{ticker_input} is currently on hold."
                jarvis_speak_clean(msg, voice_language)
                st.session_state.last_spoken_signal = current_signal_text
    ...
    elif app_mode == "ऑटो स्कैन मोड":
        sel_group = "Crypto Majors 🪙" if asset_class == "Crypto" else st.sidebar.selectbox("ग्रुप चुनें", ["Nifty Bank 🏦", "Penny Stocks 🪙"])
        res = []
