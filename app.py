...
def log_trade(ticker, signal, price, sl, tp, qty, news_score, pattern):
    try:
        ...
        with open('trade_log.csv', 'a', newline='', encoding='utf-8') as f:
            ...
            writer.writerow([datetime.now(pytz.timezone('Asia/Kolkata')).strftime('%Y-%m-%d %H:%M'),
...
try:
    if nav == "📊 Dashboard":
        ...
        if c2.button("🔍 Search", use_container_width=True) and q:
            ...
            df, err_msg = fetch_data(query)   # नया टपल अनपैकिंग
            if df is None:
                st.error(err_msg)
            else:
        ...
        st.caption("App Version: 2.0.0 (Production Ready)")
