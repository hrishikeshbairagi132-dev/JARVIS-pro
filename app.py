...
@st.cache_data(show_spinner=False)
def fetch_data(ticker, ac, period="60d"):   # ✅ 60 दिन कर दिया
    try:
...
if st.sidebar.button("🚀 JARVIS V20 START", use_container_width=True):
    ...
    df = fetch_data(ticker_input, asset_class, period="60d")
    if df is None or len(df)<30:
        st.error("❌ पर्याप्त डेटा नहीं। (Yahoo 15m data केवल 60 दिन)")
        st.stop()
    ...
    # Backtest (60 दिन, SL/TP hit logic)
    with st.expander("📈 Historical Win-Rate (60 दिन) - Realistic"):
        if asset_class == "Equity (NSE)":
            bt = fetch_data(ticker_input, asset_class, period="60d")
