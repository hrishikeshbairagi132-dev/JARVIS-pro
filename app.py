...
# सेशन स्टेट्स (Zero Loss Control)
...
# ============================================================
# 🔄 3. HIGH-SPEED BACKUP ROUTE (Google Finance Extraction)
# ============================================================
def fetch_backup_google_data(ticker: str, asset_class: str) -> Optional[pd.DataFrame]:
    try:
        ...
        if response.status_code == 200 and 'data-last-price="' in response.text:
            ...
            dates = pd.date_range(end=datetime.now(LOCAL_TZ), periods=60, freq='D')
...
# ============================================================
# 🛡️ 4. TECHNICAL INDICATORS SYSTEM
# ============================================================
def _add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        # कॉलम नामों को क्लीन और स्टैंडर्ड फॉर्मेट में लाना
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = [col[0].capitalize() for col in df.columns]
        else:
            df.columns = [str(c).capitalize() for c in df.columns]

        # ज़ीरो वॉल्यूम हैंडलिंग
        ...
        # अगर इंट्राडे डेटा न हो तो सिंपल क्युमुलेटिव कनवर्टर
        if df['VWAP'].isna().all():
...
# ============================================================
# 🔍 5. PATTERN INTERPRETER & BACKTEST MATRIX
# ============================================================
def detect_patterns_rolling(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        for i in range(10, len(df)):
            win = df['Close'].iloc[max(0, i-10):i]
...
# ============================================================
# 📡 6. ERROR FREE UNPACKING LIVE ROUTER (Fixed Row 166)
# ============================================================
def fetch_data(ticker_query: str, asset_type: str) -> Tuple[Optional[pd.DataFrame], str]:
    # यूजर इनपुट को क्लीन करना ताकि एरर न आए
    ticker = ticker_query.upper().strip().replace(" ", "")
    if not ticker:
        return None, "खाली सिंबल नाम दर्ज किया गया है।"
    ...
    # रूट 1: याहू फाइनेंस 15-मिनट इंटरवल (इंट्राडे)
    try:
    ...
    # रूट 2: याहू फाइनेंस डेली डेटा बैकअप (अगर 15m ब्लॉक हो)
    try:
    ...
    # रूट 3: गूगल फाइनेंस स्क्रैपर बैकअप
    df_backup = fetch_backup_google_data(ticker, asset_type)
    if df_backup is not None and not df_backup.empty:
        processed = _add_indicators(df_backup)
        if not processed.empty: return processed, "SUCCESS"
        
    return None, f"❌ '{ticker}' के लिए लाइव मार्केट डेटा लोड नहीं हो सका। कृपया टिकर कोड जांचें।"

# ============================================================
# 🎰 7. OPTION CHAIN & LOG ENGINE
# ============================================================
...
def run_analysis():
    ...
    # लाइन 166 फिक्स: अब दो वैरियेबल्स के साथ सेफ अनपैकिंग होगी
    ...
    if app_mode == "सिंगल स्टॉक Analysis":
        ...
        conf_score = (30 if 40 <= last['RSI'] <= 70 else 0) + (40 if f_score > 50 else 0) + (30 if last['vol_ratio'] > 1.1 else 0)
        ...
        sel_group = "Crypto Majors 🪙" if "CRYPTO" in asset_class.upper() else st.selectbox("ग्रुप चुनें", ["Nifty Bank 🏦", "Penny Stocks 🪙"])
        res = []
