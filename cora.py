...
def get_yfinance_ticker(user_ticker):
    ticker = user_ticker.strip().upper()
    if ticker.endswith(".NS") or ticker.endswith(".BO") or "-USD" in ticker:
        return ticker
    if ticker in CRYPTO_MAP or search_crypto_id(ticker):
        return ticker + "-USD"
    return ticker + ".NS"

def _fetch_uncached(ticker):
    """Quick live price (no cache)."""
    query = ticker.strip().upper()
    price = None
    crypto_id = CRYPTO_MAP.get(query) or search_crypto_id(query)
    if crypto_id:
        try:
        ...
        yticker = get_yfinance_ticker(query)
        try:
...
def fetch_data(user_ticker):
    """
    Returns (DataFrame, error_message).
    DataFrame is None if error, else contains indicators.
    """
    try:
        ticker = get_yfinance_ticker(user_ticker)
        logger.info(f"Fetching data for {user_ticker} -> {ticker}")

        df = yf.Ticker(ticker).history(period="60d", interval="15m")
        if df.empty or len(df) < 60:
            return None, f"❌ {user_ticker} के लिए पर्याप्त डेटा उपलब्ध नहीं है (टिकर जाँचें या सर्वर बंद है)"
        ...
        if not all(c in df.columns for c in required):
            return None, f"❌ {user_ticker} के डेटा में आवश्यक कॉलम नहीं मिले"
        ...
        logger.error(f"Error fetching data for {user_ticker}: {e}")
        return None, f"❌ {user_ticker} के लिए नेटवर्क/API एरर: {str(e)}"
...
def detect_patterns(df):
    ...
    engulfing = (df['open'] <= df['close'].shift(1)) & (df['close'] >= df['close'].shift(1)) & (body > body.shift(1))
